...
import { Search, Compass, GitCompare, BookOpen, Bookmark, Crown, LogOut, User, CreditCard, ChevronDown, BarChart3, HelpCircle, Menu, X } from 'lucide-react';
...
export const DashboardLayout = () => {
    ...
    <>
      ...
      <div className="flex-1 py-3 px-3 flex flex-col gap-0.5 overflow-y-auto">
        ...
        <div className="mt-2.5 pt-2.5 border-t border-black/10">
          ...
          {[{ name: 'Pricing', icon: CreditCard, path: '/pricing' }].map((item) => (
            <NavLink
              key={item.name}
              to={item.path}
              className={({ isActive }) =>
  ...
  return (
    <div className="min-h-screen flex bg-slate-50">
      ...
      <aside className="hidden lg:flex w-60 border-r-2 border-black bg-white flex-col fixed h-full z-20">
        <SidebarContent />
      </aside>
      ...
      <aside
        className={cn(
          'fixed top-0 left-0 h-full w-72 bg-white border-r-2 border-black flex flex-col z-40 transition-transform duration-300 ease-in-out lg:hidden',
      ...
      >
        ...
        <button
          onClick={() => setSidebarOpen(false)}
          className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center rounded-lg border-2 border-black bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] z-10"
        >
          <X className="w-4 h-4" />
        </button>
        <SidebarContent />
      ...
      <main className="flex-1 lg:ml-60 flex flex-col min-h-screen">
        ...
        {/* ── Topbar ── */}
        <header className="h-14 sm:h-16 bg-white border-b-2 border-black sticky top-0 z-10 flex items-center justify-between px-3 sm:px-6 gap-3">

          {/* Hamburger (mobile only) */}
          <button
            onClick={() => setSidebarOpen(true)}
            className="lg:hidden flex-shrink-0 w-9 h-9 flex items-center justify-center rounded-lg border-2 border-black bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all hover:bg-slate-50"
          >
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-500 font-black hidden sm:block bg-slate-100 border border-black/10 px-2 py-1 rounded">{companiesCount} cos</span>
            {user && isPro && (
              <span className="flex items-center gap-1 text-xs font-black text-amber-950 bg-amber-100 border border-black px-2 py-1 rounded-full shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]">
                <Crown className="w-3 h-3 text-amber-500" /> <span className="hidden sm:inline">Pro</span>
              </span>
            )}
            {!user ? (
              <button
                onClick={() => navigate('/login')}
                className="text-xs font-black text-black bg-white hover:bg-slate-50 border-2 border-black px-3 py-1.5 rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all"
              >
                Login
              </button>
            ) : null}
            {!isPro && (
              <button
                onClick={() => navigate('/pricing')}
                className="text-xs font-black text-black bg-[#48d2e1] hover:bg-[#35d5e5] border-2 border-black px-3 py-1.5 rounded-lg flex items-center gap-1 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all"
              >
                <Crown className="w-3 h-3" /> <span className="hidden sm:inline">Upgrade</span>
              </button>
            )}
          </div>
        </header>
